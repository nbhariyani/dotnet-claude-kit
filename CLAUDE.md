# nestjs-claude-kit — Development Instructions

> These instructions are for developing THIS repository. For user-facing project templates, see `templates/`.

## Repository Purpose

nestjs-claude-kit is an opinionated Claude Code companion for NestJS developers. It provides skills, agents, templates, knowledge documents, and a TypeScript MCP server that make Claude Code dramatically more effective for NestJS development.

## Philosophy

- **Guided over prescriptive** — We ask the right questions, then recommend the best approach with clear rationale
- **Modern NestJS only** — Target NestJS 11+ and TypeScript 5.x. No legacy patterns, no JavaScript, no CommonJS
- **Architecture-aware** — We support Feature Modules, Clean Architecture, DDD, and Modular Monolith with an advisor skill that recommends the best fit
...
2. **Patterns** — Code examples with explanation. Each pattern has:
   ...
   - Working TypeScript code (must compile conceptually)
...
- **Code examples must be modern TypeScript** — Strict mode, decorators, `const`, arrow functions, interfaces
- **No `synchronize: true` in TypeORM production** — Always use migrations
- **No direct cross-module service imports** — Use `exports[]` in NestJS modules
- **`@nestjs/config` over `process.env`** — Always use `ConfigService`
...
3. **MCP tool usage** — When to use cwm-ts-navigator tools vs reading files
...
- `nestjs-whats-new.md` — Updated per NestJS/Node.js release
- `common-antipatterns.md` — Patterns Claude should never generate
- `package-recommendations.md` — Vetted npm packages
...
The MCP server lives at `mcp/cwm-ts-navigator/`. It's a Node.js application using the `@modelcontextprotocol/sdk` and `ts-morph`.
...
- The project must handle **graceful loading** — Return "loading" status instead of errors during initialization

## Workflow Standards

How Claude should work on this repository (and any project using nestjs-claude-kit templates).
...
- Run `npm run build` and `npm test` after changes — green builds are the minimum bar
- Use `get_diagnostics` via the ts-morph MCP to catch TypeScript errors after modifications
- Ask yourself: "Would a staff NestJS engineer approve this?" — if not, iterate
...
1. Check the spec at `docs/nestjs-claude-kit-SPEC.md` for the full vision
